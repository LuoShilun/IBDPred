...
if __name__ == "__main__":
    ...
    for feature_method in ['rf', 'mi']:
        ...
        model = StackingClassifier(
            estimators=[
                ('svm', SVC(random_state=2025,C=1 if 'mi' in feature_method else 1,
                            kernel='rbf',
                            gamma=0.01 if 'mi' in feature_method else 0.1, probability=True)),
                ('xgb', XGBClassifier(random_state=2025,learning_rate=0.1 if 'mi' in feature_method else 0.05,
                                      max_depth=7 if 'mi' in feature_method else 3,
                                      gamma=0 if 'mi' in feature_method else 0.4,
                                      n_estimators=100 if 'mi' in feature_method else 100,
                                      subsample=0.8 if 'mi' in feature_method else 0.95,
                                      colsample_bytree=0.2 if 'mi' in feature_method else 0.1)),
                ('rf', RandomForestClassifier(random_state=2025, n_estimators=60 if 'mi' in feature_method else 40,
                                            max_depth=14 if 'mi' in feature_method else 12,
                                            min_samples_split=5 if 'mi' in feature_method else 7))
            ],
    ...
    plt.savefig('stacking_roc.png', dpi=300, bbox_inches='tight')
    plt.close()
