...
if __name__ == "__main__":
    ...
    for feature_method in ['rf', 'mi']:
        ...
        # 构建基学习器
        estimators = [
            ('svm', SVC(random_state=2025,C=10 if 'mi' in feature_method else 1,
                            kernel='rbf',
                            gamma=1 if 'mi' in feature_method else 0.1, probability=True)),
                ('xgb', XGBClassifier(random_state=2025,learning_rate=0.01 if 'mi' in feature_method else 0.01,
                                      max_depth=9 if 'mi' in feature_method else 9,
                                      gamma=0 if 'mi' in feature_method else 0,
                                      n_estimators=200 if 'mi' in feature_method else 100,
                                      subsample=0.8 if 'mi' in feature_method else 0.9,
                                      colsample_bytree=0.3 if 'mi' in feature_method else 0.2)),
                ('rf', RandomForestClassifier(random_state=2025, n_estimators=60 if 'mi' in feature_method else 80,
                                            max_depth=10 if 'mi' in feature_method else 14,
                                            min_samples_split=2 if 'mi' in feature_method else 3))
        ]
    ...
    # 保存结果字典到文件
    import pickle
    with open('stacking_improved_cv_results.pkl', 'wb') as f:
