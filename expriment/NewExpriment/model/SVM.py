...
# ==================== 核心函数 ====================
def svm_full_pipeline(data_type):
    """全流程建模评估函数"""
    # ---------- 数据加载 ----------
    ...
    model = SVC(random_state=2025,probability=True)

    searcher = GridSearchCV(model, param_grid, scoring='accuracy', cv=3, n_jobs=-1)
    searcher.fit(X, y)

    # ---------- 保存最佳模型 ----------
    best_model = searcher.best_estimator_
    joblib.dump(best_model, f'./ModelPKL/svm_best_{data_type}.pkl')
...
def plot_combined_roc(rf_data, mi_data):
    ...
    plt.plot(rf_data['mean_fpr'], rf_data['mean_tpr'],
             color=colors['RF'],
             label=f'(RF)SVM(AUC={rf_data["auc"]:.3f})',
             lw=2.5, linestyle='-')

    # 绘制MI数据集曲线
    plt.plot(mi_data['mean_fpr'], mi_data['mean_tpr'],
             color=colors['MI'],
             label=f'(MI-VAR)SVM(AUC={mi_data["auc"]:.3f})',
             lw=2.5, linestyle='--')
    ...
    plt.savefig('./resultsImage/combined_roc_svm.png', dpi=300, bbox_inches='tight')
    plt.close()

# ==================== 执行入口 ====================
if __name__ == "__main__":
    rf_data = svm_full_pipeline("RF")
    mi_data = svm_full_pipeline("MI")
    auto_data = svm_full_pipeline("Autoencoder")
