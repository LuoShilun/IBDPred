...
# 测试集数据路径
TEST_DATASETS = {
    "rf": "../dataset/test/rf_test.csv",
    "mi": "../dataset/test/mi_test.csv"
}

# 模型文件路径 - 简化为三种模型在两个数据集上
MODEL_FILES = {
    "软投票-MI-VAR": {
        "mi": "./ModelPKL/voting_model_mi.pkl",
    },
    "软投票-RF": {
        "rf": "./ModelPKL/voting_model_rf.pkl",
    },
    "改进Stacking-RF": {
        "rf": "./ModelPKL/model_new_rf.pkl",
    },
    "改进Stacking-MI-VAR": {
        "mi": "./ModelPKL/model_new_mi.pkl",
    },
    "加权平均融合-MI-VAR": {
        "mi": "./ModelPKL/model_mi_weighted.pkl",
    },
    "加权平均融合-RF": {
        "rf": "./ModelPKL/model_rf_weighted.pkl",
    }
}
...
def plot_combined_roc():
    ...
    # 保存图形
    plt.savefig('./resultsImage/roc_combined.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrix():
    ...
    if os.path.exists(model_path):
        try:
            ...
            # 保存图形
            plt.savefig('./resultsImage/stacking_rf_confusion_matrix.png', dpi=300, bbox_inches='tight')
            plt.close()
...
def plot_all_models_roc():
    ...
    # 定义模型名称和对应的文件路径
    models = {
        "加权平均-RF": "./ModelPKL/model_rf_weighted.pkl",
        "加权平均-MI": "./ModelPKL/model_mi_weighted.pkl",
        "软投票-RF": "./ModelPKL/voting_model_rf.pkl",
        "软投票-MI": "./ModelPKL/voting_model_mi.pkl",
        "Stacking-RF": "./ModelPKL/model_rf.pkl",
        "Stacking-MI": "./ModelPKL/model_mi.pkl",
        "改进Stacking-RF": "./ModelPKL/model_new_rf.pkl",
        "改进Stacking-MI": "./ModelPKL/model_new_mi.pkl"
    }
    ...
    # 保存图形
    plt.savefig('./resultsImage/all_models_roc_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
